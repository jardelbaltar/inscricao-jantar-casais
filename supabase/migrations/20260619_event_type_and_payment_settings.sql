-- Add event type and payment settings.

ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS is_couples_event boolean NOT NULL DEFAULT true,
...
BEGIN
  SELECT id, max_couples, is_open, requires_dish, is_couples_event
    INTO v_event
  FROM public.events
  WHERE id = p_event_id;
  ...
  IF coalesce(v_event.is_open, true) = false THEN
    RETURN jsonb_build_object('ok', false, 'error', 'EVENT_CLOSED');
  END IF;

  IF coalesce(v_event.is_couples_event, true) AND p_wedding_date IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'WEDDING_DATE_REQUIRED');
  END IF;

  IF coalesce(v_event.is_couples_event, true) = false THEN
    p_wedding_date := NULL;
  END IF;
